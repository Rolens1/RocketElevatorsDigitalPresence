...
$('input').change(function() {
    switch(selectOption.value) {
        case 'residential':
            if($("#num-floor").val() <= 20) {
                numOfElevator = Math.ceil($("#num-app").val() / 6) 
            }    
            if($('#num-floor').val() > 20){
                numOfElevator = (Math.ceil($("#num-appi").val() / 6)) * (Math.ceil($('#num-floor').val() / 20))   
            }
            break;
        case 'commercial':
            ...
            break;
        case 'corporate':
            x = (parseInt($('#num-floor').val()) + parseInt($('#numBasement').val()))
            alert(x)

            //Trouver le nombre d'ascencseur total nécessaire  -> ((Etage + Sous-sol)*MaxParEtage)/1000
            elevatorCorp = Math.ceil(x *parseInt($('#max-floor').val()) / 1000)
